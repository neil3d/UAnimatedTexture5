 ...
 *   - 单行索引行缓冲 LineBuffer（W bytes，跨帧复用）；
 *   - 当前 RGBA canvas FrameBuffer；
 *   - 按需的 DISPOSE_PREVIOUS 快照 SnapshotBuffer。
 *
 * 不再调用 DGifSlurp，也不再触碰 GIF->SavedImages —— 全部帧像素按需在
...
public:
	...
	virtual uint32 GetLoopCount() const override { return LoopCountFromFile; }

	// 已完成循环次数；NextFrame 跨过末尾时 LoopCount 会自增。
	virtual uint32 GetCompletedLoops() const override { return static_cast<uint32>(LoopCount); }

private:
	// 单帧元数据：LoadFromMemory 阶段一次性预扫缓存；运行期只读，不再触碰
	// GIF->SavedImages（流式模式下它一直为空）。
	struct FFrameMeta
	{
	...
	// DISPOSE_BACKGROUND 清为 ResolvedBgColor；
	// DISPOSE_PREVIOUS 从 SnapshotBuffer 恢复对应区域。
	...
	//   - Frames（每帧 Rect/Disposal/Delay/Transparent/Interlace）
	//   - LoopCountFromFile（NETSCAPE2.0）
	//   - bHasTransparency
...
private:
	...
	FIntRect PrevRect = FIntRect();
	int PrevTransparentColor = NO_TRANSPARENT_COLOR;

	// DISPOSE_PREVIOUS 用的画布快照；按需分配。
	TArray<FColor> SnapshotBuffer;
	...
	FColor ResolvedBgColor = FColor(0, 0, 0, 0);

	GifFileType* GIF = nullptr;
	TArray<FColor> FrameBuffer;
	FInputCursor InputCursor;

	// 单行索引位图缓冲（W bytes），跨帧复用，避免每帧 malloc。
	TArray<GifPixelType> LineBuffer;

	// 每帧元数据；由 ScanMetadata 一次性填充，运行期只读。
	TArray<FFrameMeta> Frames;

	// NETSCAPE2.0 应用扩展中声明的循环次数；0 = 无限循环 / 文件未声明。
	uint32 LoopCountFromFile = 0;
